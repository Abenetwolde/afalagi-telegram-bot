...
const createSubmissionKeyboard = () =>
...
exports.questionnaireScene = new telegraf_1.Scenes.WizardScene(
  "questionnaire",
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (ctx.wizard.state.questions.length > 0) {
        ...
        await ctx.reply(
          ctx.wizard.state.questions[0].text,
          telegraf_1.Markup.keyboard(["Skip"]).oneTime(),
        );
    ...
    try {
      if (ctx.callbackQuery && "data" in ctx.callbackQuery) {
        if (ctx.callbackQuery.data === "review") {
          ...
          if (ctx.wizard.state.questions.length > 0) {
            logger_service_1.logger.info(
              "Sending first question for new submission:",
              ctx.wizard.state.questions[0].text,
            );
            await ctx.reply(
              ctx.wizard.state.questions[0].text,
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (currentIndex > 0 && ctx.message && "text" in ctx.message) {
        ...
        const answerText =
          ctx.message.text !== "Skip" ? ctx.message.text : "Skipped";
    ...
    try {
      if (ctx.callbackQuery && "data" in ctx.callbackQuery) {
        if (ctx.callbackQuery.data === "back") {
          logger_service_1.logger.info(
            "Back button triggered, returning to review/new selection",
          );
          await ctx.reply(
            "You have a previous submission. Would you like to review/edit it or start a new one?",
            telegraf_1.Markup.inlineKeyboard([
  ...
  async (ctx) => {
    ...
    try {
      if (!ctx.message || !("text" in ctx.message)) {
        await ctx.reply(
          "Please enter the number of the question you want to edit:",
        );
        return ctx.wizard.selectStep(4);
      }
      ...
      logger_service_1.logger.info(
        `Selected question ${questionNumber}: ${ctx.wizard.state.questions[questionIndex].text}`,
      );
      await ctx.reply(
      ...
      logger_service_1.logger.error(
        "Error in question number input step:",
        error,
      );
      await ctx.reply("An error occurred. Please try again.");
      return ctx.scene.leave();
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (user && user.lastSubmissionId) {
        ...
        logger_service_1.logger.info(
          `Updated answer for question ${questionIndex + 1} in submission ${user.lastSubmissionId}`,
        );
      } else {
        logger_service_1.logger.warn(
          `No submission found for user ${ctx.from.id}. Creating a new one.`,
        );
        ...
        logger_service_1.logger.info(
          "New submission created with ID:",
          submission.id,
        );
      }
      await ctx.reply(
