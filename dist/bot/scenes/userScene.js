...
const createSubmissionSelectionKeyboard = (submissions) => {
    const buttons = submissions.map((s, index) => telegraf_1.Markup.button.callback(`Submission ${index + 1} (${s.status})`, `select_${s.id}`));
...
const createActionKeyboard = () => telegraf_1.Markup.inlineKeyboard([
...
const createConfirmationKeyboard = () => telegraf_1.Markup.inlineKeyboard([
...
exports.userScene = new telegraf_1.Scenes.WizardScene('user', 
...
async (ctx) => {
    ...
    try {
        ...
        await ctx.reply('Your submissions:\n' + formatSubmissionList(submissions) + '\n\nSelect a submission to view details:', createSubmissionSelectionKeyboard(submissions));
        ...
        if (ctx.callbackQuery && 'data' in ctx.callbackQuery) {
            ...
            if (data.startsWith('select_')) {
                ...
                if (submission.status === 'pending') {
                    await ctx.reply(formatSubmissionDetails(submission, ctx.wizard.state.questions) + '\n\nChoose an action:', { reply_markup: createActionKeyboard().reply_markup, parse_mode: 'Markdown' });
                }
                else {
                    await ctx.reply(formatSubmissionDetails(submission, ctx.wizard.state.questions) + '\n\nThis submission is not pending, so it cannot be edited or canceled.', { reply_markup: telegraf_1.Markup.inlineKeyboard([telegraf_1.Markup.button.callback('Back', 'back')]).reply_markup, parse_mode: 'Markdown' });
                }
                return ctx.wizard.next();
            }
        }
...
async (ctx) => {
    ...
    try {
        if (ctx.callbackQuery && 'data' in ctx.callbackQuery) {
            ...
            if (data === 'back') {
                await ctx.reply('Your submissions:\n' + formatSubmissionList(ctx.wizard.state.submissions) + '\n\nSelect a submission to view details:', createSubmissionSelectionKeyboard(ctx.wizard.state.submissions));
                return ctx.wizard.selectStep(1);
            }
            if (data === 'cancel' && submission.status === 'pending') {
                await ctx.reply('Are you sure you want to cancel this submission? This action cannot be undone.', createConfirmationKeyboard());
            ...
            if (data === 'confirm_cancel' && submission.status === 'pending') {
                ...
                logger_service_1.logger.info(`User ${ctx.from.id} canceled submission ${submissionId}`);
        ...
        logger_service_1.logger.error('Error handling submission actions:', error);
        await ctx.reply('An error occurred. Please try again.');
        return ctx.scene.leave();
    }
}, 
...
async (ctx) => {
    ...
    try {
        if (!ctx.wizard.state.editing) {
            await ctx.reply('Please click "Edit" to modify your answers.', { reply_markup: createActionKeyboard().reply_markup, parse_mode: 'Markdown' });
            return ctx.wizard.selectStep(2);
        }
        if (ctx.message && 'text' in ctx.message) {
            ...
            if (questionNumber >= 0 && questionNumber < ctx.wizard.state.questions.length) {
                ...
                await ctx.reply(ctx.wizard.state.questions[questionNumber].text, createBackKeyboard());
                return ctx.wizard.selectStep(4);
            }
            else {
                await ctx.reply('Invalid question number. Please enter a valid number.', { reply_markup: telegraf_1.Markup.removeKeyboard().reply_markup });
                return ctx.wizard.selectStep(3); // Stay in this step for valid input
            }
        }
        await ctx.reply('Please enter the number of the question to edit.', { reply_markup: telegraf_1.Markup.removeKeyboard().reply_markup });
        return ctx.wizard.selectStep(3);
    }
    catch (error) {
...
async (ctx) => {
    ...
    try {
        if (ctx.message && 'text' in ctx.message) {
            ...
            logger_service_1.logger.info(`User ${ctx.from.id} updated submission ${selectedSubmissionId}`);
            ...
            if (updatedSubmission) {
                await ctx.reply('Answer updated!\n\nUpdated answers:\n' +
