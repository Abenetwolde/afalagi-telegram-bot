...
// Admin Scene
exports.adminScene = new telegraf_1.Scenes.WizardScene(
  "admin",
  // Step 0: Main menu
  async (ctx) => {
    ...
    try {
      if (
        ...
        for (const user of users) {
          ...
          const nameAnswer =
            lastSubmission?.answers.find((a) => a.questionId.key === "name")
              ?.answer || "Unknown";
    ...
    try {
      if (
        ...
        for (const submission of submissions) {
          ...
          const nameAnswer =
            submission.answers.find((a) => a.questionId.key === "name")
              ?.answer || "Unknown";
        ...
        submissions.forEach((submission) => {
          ctx.scene.session.actionHandler = ctx.scene.action(
            `view_${submission._id}`,
            async (ctx) => {
              ...
              await ctx.reply(details, {
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                  [
                    telegraf_1.Markup.button.callback(
                      "Approve",
                      `approve_${submission._id}`,
                    ),
                  ],
                  [
                    telegraf_1.Markup.button.callback(
                      "Reject",
                      `reject_${submission._id}`,
                    ),
                  ],
                  [
                    telegraf_1.Markup.button.callback(
    ...
    try {
      if (ctx.callbackQuery && "data" in ctx.callbackQuery) {
        ...
        if (data.startsWith("approve_") || data.startsWith("reject_")) {
          ...
          await submission_model_1.Submission.updateOne(
            { _id: submissionId },
    ...
    try {
      ...
      if (ctx.message && "text" in ctx.message) {
        ...
        if (await question_model_1.Question.findOne({ key })) {
          await ctx.reply(
            "This key already exists. Please enter a unique key:",
          );
          return ctx.wizard.selectStep(4);
        }
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (ctx.callbackQuery && "data" in ctx.callbackQuery) {
        const category =
          ctx.callbackQuery.data === "category_personal"
            ? "personal"
            : "partner";
      ...
      logger_service_1.logger.error(
        "Error in add question text/category step:",
        error,
      );
      await ctx.reply("An error occurred. Please try again.");
      return ctx.scene.leave();
