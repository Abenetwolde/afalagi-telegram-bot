...
async (ctx) => {
    ...
    try {
        if (ctx.callbackQuery && 'data' in ctx.callbackQuery && ctx.callbackQuery.data === 'view_users') {
            ...
            await ctx.reply(message, { parse_mode: 'Markdown', reply_markup: telegraf_1.Markup.inlineKeyboard([
    ...
    try {
        if (ctx.callbackQuery && 'data' in ctx.callbackQuery && ctx.callbackQuery.data === 'view_applications') {
            ...
            for (const submission of submissions) {
                ...
                const nameAnswer = submission.answers.find((a) => a.questionId.key === 'name')?.answer || 'Unknown';
